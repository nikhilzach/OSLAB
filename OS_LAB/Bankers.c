...
{
    /*Pointing index variable of safe sequence to beginning of the safe sequence array*/
    sequence_index = 0;
    ...
    for(k = 0; k < no_of_processes; k++)
    {
        for(i = 0; i < no_of_processes; i++)
        {
            if(finish[i] == 0)
            {
                /*flag is set to 1 if any of the instance need of a process 
...
void main()
{
    ...
    printf("P%d", safe_sequence[no_of_processes - 1]);
}
